...
import { loadTeamRecord, saveTeamRecord, upsertTeamMember } from "pi-claude-runtime-core/team-state";
...
import { parseDetachedBackgroundPayload, parseDetachedControlMessage } from "./detached-background-schemas.js";
...
function resolveModel(modelRegistry: ModelRegistry, label: string | undefined): Model<any> | undefined {
  ...
  return modelRegistry.getAll().find((model) => `${model.provider}/${model.id}` === raw || model.id === raw || model.name === raw);
}

async function loadMailboxMessages(mailboxFile: string, consumedLines: number): Promise<{ lines: number; messages: ReturnType<typeof parseDetachedControlMessage>[] }> {
  try {
    ...
    return {
      lines: lines.length,
      messages: unread.map((line) => parseDetachedControlMessage(JSON.parse(line))),
    };
  } catch {
...
async function main(): Promise<void> {
  ...
  const payload = parseDetachedBackgroundPayload(JSON.parse(await fs.promises.readFile(payloadPath, "utf-8")));
  const authStorage = AuthStorage.create();
  const modelRegistry = ModelRegistry.create(authStorage);
  const currentModel = resolveModel(modelRegistry, payload.currentModel);
  const sessionManager = SessionManager.open(payload.sessionFile, payload.sessionDir, payload.cwd);
  const sessionId = sessionManager.getSessionId();
  const managedTaskRegistry = new ManagedTaskRegistry(payload.cwd);
  ...
  const permissions: AgentPermissionConfig = {
    ...
    ...(payload.disallowedTools ? { disallowedTools: payload.disallowedTools } : {}),
    ...(payload.allowedDirectories ? { allowedDirectories: payload.allowedDirectories } : {}),
    ...(payload.allowedSkills ? { allowedSkills: payload.allowedSkills } : {}),
    ...(payload.disallowedSkills ? { disallowedSkills: payload.disallowedSkills } : {}),
    ...(payload.agent.permissionMode ? { permissionMode: payload.agent.permissionMode } : {}),
  };

  const { runtime, model } = await createSdkAgentRuntime({
    defaultCwd: payload.cwd,
    agent: payload.agent,
    modelRegistry,
    currentModel,
    ...
    permissions,
    customTools: payload.kind === "teammate" && payload.teamName
      ? buildClaudeTodoCustomTools(payload.cwd, payload.teamName, {
      ...
      : undefined,
    managedPlanMode: payload.agent.permissionMode === "plan"
      ? {
          ...(payload.managedPlanModeInitialReason ? { initialReason: payload.managedPlanModeInitialReason } : {}),
  ...
  const runningRecord = {
    ...
    ...(payload.disallowedSkills ? { disallowedSkills: payload.disallowedSkills } : {}),
    ...(payload.agent.permissionMode ? { permissionMode: payload.agent.permissionMode } : {}),
    ...(payload.agent.effort !== undefined ? { effort: payload.agent.effort } : {}),
    ...(payload.agent.mcpServers ? { mcpServers: payload.agent.mcpServers } : {}),
    ...(payload.agent.requiredMcpServers ? { requiredMcpServers: payload.agent.requiredMcpServers } : {}),
    ...(payload.agent.hooks ? { hooks: payload.agent.hooks } : {}),
    ...(payload.agent.isolation ? { isolation: payload.agent.isolation } : {}),
    ...(resolveModelLabel(currentModel) || model ? { model: payload.modelOverride ?? resolveModelLabel(currentModel) ?? model } : {}),
    detached: true,
  ...
  if (payload.kind === "teammate" && payload.teamName) {
    const team = await loadTeamRecord(payload.cwd, payload.teamName);
    if (team) {
      await saveTeamRecord(payload.cwd, upsertTeamMember(team, toTeamMemberRecord(runningRecord)));
    }
  }
  ...
  const mailboxTimer = setInterval(async () => {
    ...
    for (const message of update.messages) {
      ...
      await runtime.session.sendUserMessage(message.content, { deliverAs: "steer" });
    }
  }, 500);
  mailboxTimer.unref?.();
  ...
  try {
    ...
    const finalStatus = result.exitCode !== 0 || result.stopReason === "error" || result.stopReason === "aborted"
      ? abortController.signal.aborted ? "interrupted" : "failed"
      : "completed";
    ...
    await updateNamedAgentRecordStatusOnDisk({
      ...
      mutate: (record) => ({
        ...
        lastError: finalStatus === "completed" ? undefined : (result.errorMessage ?? result.stderr ?? record.lastError),
    ...
    await managedTaskRegistry.noteTerminal({
      record: {
        ...
        background: false,
        lastCompletedAt: completedAt,
        lastError: finalStatus === "completed" ? undefined : (result.errorMessage ?? result.stderr ?? undefined),
      },
      result,
      wasBackground: true,
    });
    if (payload.kind === "teammate" && payload.teamName) {
      const team = await loadTeamRecord(payload.cwd, payload.teamName);
      if (team) {
        await saveTeamRecord(payload.cwd, upsertTeamMember(team, toTeamMemberRecord({
          ...runningRecord,
          status: finalStatus,
          lastCompletedAt: completedAt,
          lastError: finalStatus === "completed" ? undefined : (result.errorMessage ?? result.stderr ?? undefined),
    ...
    await appendDetachedOutboxEvent({
      ...
      event: {
        ...
        ...(result.errorMessage ?? result.stderr ? { error: result.errorMessage ?? result.stderr } : {}),
